...
export function DashboardPanel({ onNavigate }: DashboardPanelProps) {
  ...
  const connectedDevices = devices.filter(d => d.connection?.status === 'connected').length
  const totalDevices = devices.length

  // Get recent missions (sorted by createdAt, most recent first)
  const recentMissions = [...missions]
    .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
    .slice(0, 3)

  // Get active automations list
  const activeAutomationsList = automations
    .filter(a => a.status === 'active')
    .slice(0, 3)

  const isLoading = missionsLoading || automationsLoading || devicesLoading
  return (
    <div className="flex h-full flex-col gap-4 overflow-x-hidden overflow-y-auto px-1 pt-12 md:gap-6 md:px-0 md:pt-0">
      {/* Hero Banner */}
      <div className="relative min-w-0 rounded-xl border border-primary/20 bg-gradient-to-br from-primary/10 via-background to-info/5 p-4 md:rounded-2xl md:p-6">
        <div className="absolute -right-20 -top-20 hidden h-64 w-64 rounded-full bg-primary/5 blur-3xl md:block" />
        <div className="absolute -bottom-10 -left-10 hidden h-40 w-40 rounded-full bg-info/5 blur-2xl md:block" />
        ...
        <div className="relative flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-sm text-muted-foreground md:text-base">
              Your AI agent is actively working on <span className="font-medium text-primary">{isLoading ? '...' : activeMissions} mission{activeMissions !== 1 ? 's' : ''}</span> and
              running <span className="font-medium text-primary">{isLoading ? '...' : activeAutomations} automation{activeAutomations !== 1 ? 's' : ''}</span>. You've saved{" "}
          ...
          <div className="hidden items-center gap-6 xl:flex">
            ...
            <div className="text-center">
              {devicesLoading ? (
                ...
                <div className="text-3xl font-bold text-foreground">{connectedDevices}/{totalDevices}</div>
              )}
      ...
      <div className="grid min-w-0 grid-cols-2 gap-2 sm:gap-3 md:gap-4 lg:grid-cols-5">
          ...
          <CardContent className="p-3 md:p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                {missionsLoading ? (
                  <Skeleton className="h-6 w-8 mt-1 md:h-8 md:w-10" />
                ) : (
                  <p className="mt-1 text-xl font-bold text-foreground md:text-2xl">{activeMissions}</p>
                )}
              </div>
        ...
        <Card className="border-border bg-card transition-colors hover:border-accent/30">
          <CardContent className="p-3 md:p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                {automationsLoading ? (
                  ...
                  <p className="mt-1 text-xl font-bold text-foreground md:text-2xl">{activeAutomations}</p>
                )}
              </div>
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-accent/10 md:h-10 md:w-10 md:rounded-xl">
                <RotateCw className="h-4 w-4 text-accent md:h-5 md:w-5" />
              </div>
            </div>
            <div className="mt-2 hidden items-center gap-1 text-xs sm:flex">
        ...
        <Card className="border-border bg-card transition-colors hover:border-info/30">
          <CardContent className="p-3 md:p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-medium text-muted-foreground">Devices Online</p>
                {devicesLoading ? (
                  <Skeleton className="h-6 w-12 mt-1 md:h-8 md:w-16" />
                ) : (
                  <p className="mt-1 text-xl font-bold text-foreground md:text-2xl">{connectedDevices}/{totalDevices}</p>
                )}
              </div>
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-info/10 md:h-10 md:w-10 md:rounded-xl">
                <Monitor className="h-4 w-4 text-info md:h-5 md:w-5" />
              </div>
            </div>
            <div className="mt-2 hidden items-center gap-1 text-xs sm:flex">
              {devicesLoading ? (
                ...
                <span className="text-muted-foreground">{totalDevices - connectedDevices} device{totalDevices - connectedDevices !== 1 ? 's' : ''} offline</span>
              )}
            </div>
          </CardContent>
        </Card>

        <Card className="border-border bg-card transition-colors hover:border-warning/30">
          <CardContent className="p-3 md:p-4">
            <div className="flex items-center justify-between">
              <div>
              ...
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-warning/10 md:h-10 md:w-10 md:rounded-xl">
                <Zap className="h-4 w-4 text-warning md:h-5 md:w-5" />
              </div>
            </div>
            <div className="mt-2 hidden items-center gap-1 text-xs sm:flex">
              <ArrowUpRight className="h-3 w-3 text-primary" />
        ...
        <Card className="col-span-2 border-border bg-card transition-colors hover:border-success/30 lg:col-span-1">
          <CardContent className="p-3 md:p-4">
            <div className="flex items-center justify-between">
              <div>
              ...
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-success/10 md:h-10 md:w-10 md:rounded-xl">
                <Clock className="h-4 w-4 text-success md:h-5 md:w-5" />
              </div>
            </div>
            <div className="mt-2 hidden items-center gap-1 text-xs sm:flex">
              <ArrowUpRight className="h-3 w-3 text-primary" />
                ...
                <AreaChart data={activityData}>
                  <defs>
                    <linearGradient id="activityGradient" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="oklch(0.85 0.2 155)" stopOpacity={0.3} />
            ...
            <div className="h-40 md:h-48">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={missionProgressData}>
                  ...
                  />
                  <Bar dataKey="completed" fill="oklch(0.85 0.2 155)" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="active" fill="oklch(0.6 0.2 250)" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
                ...
                >
                  <div
                    className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${
                      mission.status === "completed" ? "bg-primary/10" : "bg-info/10"
                    }`}
                  >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium text-foreground">{mission.goal}</p>
                    <p className="text-xs text-muted-foreground">
                      {new Date(mission.createdAt).toLocaleDateString()}
                    </p>
                  </div>
                  {mission.status !== "completed" && mission.progress > 0 && (
        ...
        <Card className="border-border bg-card">
          ...
          <CardContent className="space-y-3">
            {automationsLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-6 text-center">
                <RotateCw className="h-8 w-8 mb-2 text-muted-foreground" />
                <p className="text-sm text-muted-foreground">No active automations</p>
              </div>
            ) : (
              activeAutomationsList.map((automation) => (
                <div
                  key={automation.id}
                  className="flex cursor-pointer items-center gap-3 rounded-lg p-2 transition-colors hover:bg-secondary/50"
                  onClick={() => onNavigate?.("automations")}
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {automation.trigger.type === 'cron' && `Schedule: ${automation.trigger.expression}`}
                      {automation.trigger.type === 'event' && `Event: ${automation.trigger.eventName}`}
                      {automation.trigger.type === 'webhook' && 'Webhook trigger'}
              ...
              devices.slice(0, 3).map((device) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs capitalize text-muted-foreground">{device.platform}</p>
                  </div>
                  <div className="flex shrink-0 items-center gap-1.5">
                    <div
                      className={`h-2 w-2 rounded-full ${
                        device.connection?.status === "connected" ? "bg-primary" : "bg-muted-foreground"
                      }`}
                    />
                    <span
                      className={`text-xs capitalize ${
                        device.connection?.status === "connected" ? "text-primary" : "text-muted-foreground"
                      }`}
                    >
